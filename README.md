...
hide rule과 별개로 visible path의 mutability policy를 제어하는 selective readonly rule을 지원해야 한다. 현재 소스에는 `--readonly-rule` 입력 surface가 있지만, 최종 user-facing 계약과 검증 범위는 아직 확정/완료로 주장하지 않는다.
...
고려 가능한 향후 정책 모델(아직 미구현/미검증/TBD):

- `readonly-root` + `allowWrite` carve-out 같은 다른 기본값/override 조합은 향후 설계 대상으로는 열어둘 수 있다.
- 하지만 현재 문서의 목표 계약, CLI syntax, 검증 evidence를 그런 정책으로 확정해서 읽으면 안 된다.
...
### Rule input 정규화: 현재 구현 vs 고려 중인 방향

이 저장소 문서는 hide rule과 future selective readonly rule의 path-like 입력 surface를 설명할 때 현재 구현과 고려 중인 target behavior를 분리해서 기록한다.

현재 구현:

- exact path 입력: exact hide rule은 이미 virtual root 기준 절대경로(`/home/<user>/.ssh` 같은 형태)여야 한다.
- supported pattern 입력: glob grammar는 현재 `**/<basename>` 또는 `**/*.<suffix>` 계열 literal token만 허용한다. 대표 예시는 `**/.env`, `**/*.pem`, `**/*.key`, `**/*.lock`이다.
- fail-fast/unsupported 입력: relative exact path, leading `~`, `~/...`, `~user`는 현재 구현되지 않았고 입력 시 fail-fast 에러 대상이다.
- fail-fast/unsupported pattern 입력: relative/tilde prefixed glob 예시인 `./fixtures/**/*.pem`, `~/fixtures/**/*.pem`, 그리고 prefix 내부에 wildcard가 섞이는 더 넓은 glob은 현재 unsupported다.
...
- CLI 인자 파싱이 구현되어 있다: `<source-root> <mount-root>`, 반복 `--hide`, global `--readonly` bool, 반복 `--readonly-rule`. 다만 selective readonly rule surface는 소스에 존재해도 현재 문서의 live verification evidence는 아직 global `--readonly` 중심으로만 정리돼 있다.
- lexical virtual path normalization과 symlink target 해석이 구현되어 있다.
- hide matcher가 구현되어 있다: virtual root 기준 절대경로 exact rule, directory prefix hiding, `**/<basename>` / `**/*.<suffix>` 형태 glob
- hidden/readonly guard 분류가 구현되어 있다: hidden path는 `ENOENT`, global `--readonly` 활성 시 visible mutation은 `EROFS`
- relative exact path, leading `~`, `~/...`, `~user`, relative/tilde prefixed glob 입력에 대한 확장/정규화는 아직 구현되지 않았고 current examples/tests도 이미 절대화된 입력이나 prefix 없는 glob만 사용한다.
...
- 현재 mount-free unit test는 총 43개이며 parser/path/matcher/guard/FUSE baseline 동작, access/symlink/create regression, global `--readonly`/hidden multi-path mutation·xattr guard, symlink target hiding, `copy_file_range` visible/hidden 경로를 검증한다. selective readonly rule 전용 검증은 아직 없다.
...
최신 live FUSE smoke evidence(2026-06-10 기준): `/dev/fuse`가 존재하는 환경에서 repo-local fixture mount와 `source-root=/` whole-root mount가 성공했다. repo-local smoke는 hidden entry가 부모 listing에서 제외되고 hidden path 직접 접근이 `ENOENT`, global `--readonly` 기준 mutation이 `EROFS`로 처리됨을 확인했고, whole-root smoke는 `stat /bin/bash`, `ls /usr`, hidden `/home/spi-ca/.ssh`의 `ENOENT`, global `--readonly` 기준 `touch`의 `EROFS`를 확인했다. 또한 `unshare -UrR <mount> /bin/true`와 `unshare -UrR <mount> /bin/bash --noprofile --norc ...`가 성공해 user-namespace 기반 chroot 실행 smoke를 확인했다. 단, FUSE mount는 `nodev`이므로 `/dev/null` 같은 device-node 동작은 상위 supervisor/namespace layer에서 별도 제공해야 한다 (`docs/operations.md` 참고).
