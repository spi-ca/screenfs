...
The original proof scope kept broad prefix classification out of the fast path and only skipped per-entry scan-time policy evaluation for the trivial safe shape: `visibility.default=visible`, no user hidden rules, and no internal hidden mount-root rule. The current implementation also has later parent-local slices for `visibility.default=visible` with subtree-only hidden rules and for `visibility.default=hidden`, no user hidden rules, no internal hidden mount-root rule, and visible descriptors that are all subtree anchors; those later slices run in the shared collector for `readdirplus` and non-per-entry `readdir` modes and have no broad kept speed claim. Broader rule-sensitive shapes still fall back to the existing `entry_is_readable()` path and need matcher-descendant plus repeated/interleaved evidence before any speed claim.
