...
| current priority snapshot | [`current-next-performance-candidates.md`](current-next-performance-candidates.md) | fast-policy `readdirplus` page/scan is claim-grade, matcher-heavy no-writable mutability is rejected/not kept, the rejected path-join implementation remains archival, and current next-priority ordering is delegated to the canonical roadmap/candidate snapshot |
