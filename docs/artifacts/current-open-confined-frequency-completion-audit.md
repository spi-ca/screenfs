...
| --- | --- | --- |
| Candidate selection and status are explicit | [`../benchmarks.md`](../benchmarks.md), [`../performance-roadmap.md`](../performance-roadmap.md), and [`current-next-performance-candidates.md`](current-next-performance-candidates.md) now mark the direct opened-object `stat_child_no_follow()` path under `open_confined` and the fast-policy/cache-eligible `readdirplus` page-scan batch fast path as kept claims, moving the immediate next unfinished lane to matcher-heavy policy path. | Satisfied |
