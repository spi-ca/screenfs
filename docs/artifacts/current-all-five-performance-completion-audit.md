...
This audit is a historical completion snapshot for goal `9016b393-41c3-42ad-8d73-260c0a0b9446`; use `current-next-performance-candidates.md` and `performance-roadmap.md` for current kept/rejected status. Final review accepted the documented non-root read-side cold-cache approximation as coverage-only evidence and accepted that strict literal cold-cache is explicitly not claimed under the current non-root harness contract.
...
| `open_confined`/`openat2` surrounding-cost separation and direct stat-child claim | `src/fs.rs`, `src/fs/perf.rs`, `src/fs/guards.rs`, and `src/fs/tests/perf.rs` add behavior-preserving `open_like.pre_open_guard.*` and `open_like.post_open_revalidation.*` attribution. Regenerated `docs/artifacts/current-open-confined-surface-smoke.*` and `docs/artifacts/current-metadata-opendir-smoke.*` remain attribution context. Current canonical docs now also include the kept `docs/artifacts/open-confined-stat-child-open-path/summary.md` claim for the direct opened-object `stat_child_no_follow()` path, while `open-confined-frequency/summary.md` and `open-confined-fd-path/summary.md` remain rejected alternatives. | satisfied as coverage/instrumentation plus current kept direct stat-child speedup claim |
...
| Matcher-heavy directory/descendant path | `scripts/bench-screenfs.py` and `scripts/test_bench_screenfs_workloads.py` add `matcher-descendant-directory` with `matcher_descendant_readdir` / `matcher_descendant_readdirplus` and `--matcher-extra-rules > 0` non-empty visible descendant fixture. `docs/artifacts/current-matcher-descendant-directory-smoke.*` confirms non-zero descendant/readdir/readdirplus counters. `docs/artifacts/matcher-no-writable-mutability-fast-path/summary.md` records a no-writable mutability fast-path attempt that failed the policy-heavy/context gate and is not kept. `docs/artifacts/matcher-descendant-combined-probe/summary.md`, `docs/artifacts/matcher-subtree-only-best-candidate/summary.md`, and `docs/artifacts/matcher-visible-default-short-circuit/summary.md` record rejected broader/path-side attempts. No matcher-heavy speedup is kept yet; for current next-priority ordering, defer to `current-next-performance-candidates.md` / `performance-roadmap.md` rather than this historical completion readout. | satisfied as coverage + rejected experiment evidence |
...
- At this audit point, no kept readdirplus page/scan optimization was made in goal `9016b393-41c3-42ad-8d73-260c0a0b9446`; newer current docs separately record the scoped fast-policy/cache-eligible `readdirplus` page-scan claim.
