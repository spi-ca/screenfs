...
- `src/fs.rs`는 현재 `readdirplus` scan에만 `DirectoryChildVisibilityBatch`를 붙인다. helper는 한 request의 한 parent directory scan 동안만 살아 있고 state/handle table에 저장되지 않으며 cross-request cache가 아니다.
