...
- readonly는 family와 rule에 매칭된 visible mutation에만 적용된다. whole-mount readonly semantics가 필요하면 `readonly-root-allowwrite` family를 empty `allow_write`와 함께 사용한다.
- alternate policy family `readonly-root-allowwrite`에서도 hidden `ENOENT` 우선과 handler-level 판정은 유지돼야 한다.
...
- multi-path mutation은 관련된 source/target/parent를 각각 다시 검사해 selective readonly 또는 carve-out affected coordinate를 판정한다.
...
- whole-mount readonly smoke와 path-scoped selective readonly 계약을 혼동하지 않았는가
