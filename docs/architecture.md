...
이 문서는 현재 계획서(`README.md`, `docs/requirements.md`, `docs/design.md`, `docs/operations.md`)와 현재 코드(`src/*.rs`)를 함께 기준으로 정리한 아키텍처 요약이다. bare slashless glob의 cwd-anchored `./<pattern>` direct-child semantics와 prefixless recursive shorthand(`**/*.pem`, `**/.env.*`, `**/id_*`)의 cwd-anchored target semantics는 모두 현재 Rust 구현·테스트·fresh smoke evidence가 갖춰진 current contract다. goal 612c03c6에서 추가된 anchored direct-child wildcard-all(`/dir/*`, `./dir/*`, `~/dir/*`)과 trailing subtree shorthand(`/dir/**`, `./dir/**`, `~/dir/**`)은 현재 Rust 구현·테스트·fresh smoke evidence가 갖춰진 current contract다. goal 38ca1993와 goal 612c03c6의 refresh 상태는 `docs/operations.md`를 따른다.
...
- 문서상 shared grammar의 supported glob은 prefix 없는 recursive tail(`**/*.pem`, `**/.env.*`, `**/id_*`, `**/.env`), 그 explicit root-anchor/anchored recursive form(`/**/*.pem`, `./fixtures/**/*.pem`, `/a/**/*.txt` 등), same-anchor subtree shorthand(`/dir/**`, `./dir/**`, `~/dir/**`), bare slashless glob(`*.pem`, `*.key`, `.env.*`, `id_*`)의 direct-child shorthand, normalized-prefix direct-child wildcard-all form(`/dir/*`, `./dir/*`, `~/dir/*`), normalized-prefix direct-child basename-prefix/suffix form, recursive literal descendant-subtree form까지 포함한다. bare slashless glob `<pattern>`은 launch process cwd를 `source_root` relative normalized prefix로 rebase한 `./<pattern>` direct-child shorthand이며, prefixless recursive tail `**/<pattern>`은 같은 cwd anchor의 `./**/<pattern>` recursive shorthand target이다. `/dir/**`는 same-anchor subtree rule과 정확히 동등해야 하고 `/dir/*`는 immediate child와 matched-child descendants에만 적용돼야 한다. current Rust 구현·테스트·fresh smoke evidence는 pre-existing cwd-anchor/direct-child 및 recursive family와 explicit root-anchor `/**/<pattern>` distinction을 `docs/operations.md` 기준으로 확인하며, 새 anchored shorthand evidence는 source tests와 `docs/artifacts/current-compatibility-pattern-smoke-transcript.md`가 제공한다.
...
- current contract대로 `**/*.pem`가 launch cwd anchor에서 `./**/*.pem`과 동등하게 normalize되고, 같은 anchor의 `*.pem`/`./*.pem`가 더 구체적인 direct-child containment/nested-override 관계로 처리되는지, same-anchor `/dir/**`가 `/dir` subtree rule과 정확히 동등한지, `/dir/*`가 `/dir/*.pem`보다 넓고 `/dir`/`/dir/**`보다 좁은 direct-child wildcard-all 관계로 문서화됐는지, 그리고 새 anchored shorthand의 fresh source/live evidence가 source tests와 current compatibility smoke로 구분됐는가
