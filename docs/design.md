...
- fast path는 세 shape로만 제한한다: (1) `visibility.default=visible` + no hidden/internal-hidden rules인 `AllVisible`, (2) `visibility.default=visible` + subtree-only hidden rules에서 현재 parent의 direct hidden-child frontier를 분류하고 visible descendant/direct-child carve-out을 bridge child로 반영하는 parent-local hidden-subtree mode, (3) `visibility.default=hidden` + no hidden/internal-hidden rules + visible subtree descriptor만 있는 parent-local visible-subtree mode. internal hidden rule, non-subtree hidden rule, 또는 default-hidden mode의 non-subtree visible rule처럼 rule-sensitive shape는 즉시 `PerEntry` fallback으로 돌아간다.
- parent-local mode는 현재 parent의 direct child frontier만 분류한다. visible-subtree mode의 fully visible child는 바로 통과시키고 bridge-visible child는 directory일 때만 readable로 취급한다. hidden-subtree mode는 direct hidden child만 제외하되, 그 hidden child 아래에 visible descendant carve-out이 있을 수 있으면 해당 child directory를 bridge-visible child로 남긴다. deeper hidden descendant 또는 unrelated subtree는 현재 directory entry를 계속 visible로 둔다. parent의 direct child가 아니어서 helper가 확정할 수 없는 path만 기존 matcher 판정으로 되돌린다.
...
- 현재 `src/fs/state.rs`의 `FileHandle`은 backing `Arc<File>`, requested user-visible access mode, experimental `writeback_internal_reads` marker, `FileIoGuardCache`, `flush_needs_sync`를 저장한다. `read`/`write` entrypoint는 policy/opened-target guard를 먼저 실행해 hidden `ENOENT`와 readonly `EROFS` precedence를 보존한 뒤, backing fd mode에 의존하지 않고 requested access mode 기준으로 wrong-direction I/O를 `EBADF`로 거부한다. Default mode에서는 `O_WRONLY` handle의 `read`가 계속 `EBADF`다. `--experimental-writeback-cache` mode에서는 backing `O_RDWR` widening이 성공한 write-only handle에 한해 kernel writeback read-before-write를 위한 `read` request가 허용될 수 있으므로, 이 mode는 `O_WRONLY` read isolation을 security boundary로 쓰는 workload에 사용하지 않는다. plain `open(O_RDONLY)`는 `FOPEN_NOFLUSH`/`flush()` no-op을 유지하고, write-intent open (`O_WRONLY`/`O_RDWR` 또는 `O_TRUNC`/`O_CREAT` 포함)과 `create()` handle은 `FOPEN_NOFLUSH`를 쓰지 않으며 `flush()` sync 대상이 된다. focused evidence는 [`../src/fs/tests/data_mutations.rs`](../src/fs/tests/data_mutations.rs) (`write_only_open_allows_partial_overwrite_but_read_returns_ebadf`, `handle_mutators_use_requested_access_after_policy_precedence`), [`../src/fs/tests/symlinks_access_create.rs`](../src/fs/tests/symlinks_access_create.rs) (`open_honors_host_access_mode_truncate_and_append_flags`), [`src/fs/tests/perf/sync.rs`](../src/fs/tests/perf/sync.rs) (`readonly_open_uses_noflush_and_flush_skips_sync`, `write_capable_open_flushes_and_does_not_use_noflush`), [`src/fs/tests/perf/data_path.rs`](../src/fs/tests/perf/data_path.rs) (`perf_counters_record_data_path_splits_on_snapshot_guard_and_io_failures`)다. `fallocate` and `copy_file_range` also keep policy guard precedence before applying requested-access `EBADF` checks. future `writeback-cache` 또는 widened-backing-fd experiment must keep these `EBADF`/`FOPEN_NOFLUSH`/flush contracts user-request based and re-audit any additional handle-based mutation path before widening host fd capabilities.
