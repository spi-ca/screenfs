...
- large-directory page-limiting current evidence must show `readdir`/`readdirplus` returning bounded pages for small FUSE `size` budgets, reusable cookie continuation across pages, hidden/bridge-visible filtering at page boundaries, shared `readdir`/`readdirplus` cookie domain, and `readdirplus` lookup refs for returned page entries only.
...
- formal performance benchmark evidence는 [`benchmarks.md`](benchmarks.md)의 harness와 기록 규칙을 따른다. benchmark 결과는 recorded machine/kernel/backing filesystem/policy/workload에 한정하며, build time·fixture creation·mount startup을 measured workload timing에 포함하지 않는다. 기본 harness 결과는 별도 cache-control을 기록하지 않는 한 warm-cache local evidence로 읽는다.
...
- state-lock concurrency 변경을 주장하려면 lock domain, read/write lock usage, no-upgrade rule, host-I/O-outside-lock rule, readdirplus page-local lookup-ref pinning, mutation invalidation atomicity, and focused state-cache/concurrency tests를 함께 남긴다. Large-directory page-limiting evidence는 requested `size`, returned entry count/approximate serialized bytes, last-cookie continuation, hidden entry omission at page boundaries, `readdir`/`readdirplus` shared cookie domain, and lookup refs for returned `readdirplus` entries only를 포함해야 한다. Per-table lock split은 별도 design artifact 없이 current evidence로 승격하지 않는다.
