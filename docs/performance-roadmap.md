...
이 section은 metadata/open-path 관련 work의 umbrella contract다. immediate next-priority ordering은 아래 `Current next-priority shortlist`와 [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md)를 따른다. 즉, metadata/open-path가 유일한 다음 작업이라는 뜻은 아니며 matcher-heavy policy path도 명시적 priority row로 유지한다.
...
현재 post-metadata 이후 all-five-lane follow-up run은 `70a3105` (`docs(perf): 성능 후속 근거와 검증 정리`) 체크포인트 기준 완료 evidence 상태로 정리됐다: `readdirplus` page/scan, read/write follow-up beyond the completed fallback small-I/O baseline, mutation invalidation breadth-first, `open_confined` / `openat2`, matcher-heavy policy path. `920c00e` 또는 dirty-worktree provenance를 가진 linked summary는 현재 baseline이 아니라 historical rejected/coverage context다. scope label 정의와 claim-grade gate는 [`benchmarks.md`](benchmarks.md)의 `Current next-priority candidate gates`가 source of truth이고, current evidence/gap snapshot은 [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md)에 정리한다. 이 run 이후에도 next measure-first pick은 여전히 `open_confined` / `openat2` frequency row다. 이유는 checked-in fallback smoke가 여전히 attribution baseline일 뿐이고, rejected [`artifacts/open-confined-frequency/summary.md`](artifacts/open-confined-frequency/summary.md) bundle도 focused `metadata_opendir` improvement/counter reduction은 보여줬지만 claim-gate `open-confined-surface` rows를 통과하지 못해 kept speedup evidence가 아니기 때문이다. 반대로 mutation invalidation lane은 breadth-first smoke/coverage 단계이며 fresh focused subtree baseline + `invalidations.scanned_entries` reduction이 먼저 필요하고, mounted post-`FORGET` observability는 별도 driver evidence가 있거나 limitation/rejected/coverage artifact로 명시돼야 한다.
...
8. `open_confined/openat2`는 구현 전에 `--policy-preset fallback-unsafe-policy --workload-set open-confined-surface`, `--policy-preset fast-path-cache-eligible --workload-set open-confined-surface`, 필요 시 explicit `--workload metadata_opendir` baseline을 먼저 채운다
9. readdirplus attr/page 비용을 그 다음 measure-first row로 유지한다
10. mutation invalidation은 fresh focused `subtree_rename_cached_unrelated` baseline + `invalidations.scanned_entries` reduction을 먼저 요구하고, eviction breadth claim일 때만 mounted post-`FORGET` driver를 추가한다; 그 외에는 post-`FORGET` observability limitation/rejected/coverage evidence를 문서화한 scope로만 다시 implementation candidate로 올린다
