...
이 section은 metadata/open-path 관련 work의 umbrella contract와 현재 slice 상태를 함께 정리한다. immediate next-priority ordering은 아래 `Current next-priority shortlist`와 [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md)를 따른다. 즉, immediate next unfinished work는 먼저 `open_confined` / `openat2` frequency baseline completion이고, `lookup`/`getattr` same-path no-follow attr reuse는 이미 현재 코드에 들어간 implemented slice로 별도 dirty-worktree slice evidence와 함께 읽는다. 이후 `readdirplus`/matcher-heavy/read-write/mutation lane을 후순위 measure-first row로 유지한다.
...
현재 post-metadata 이후 all-five-lane follow-up run은 `70a3105` (`docs(perf): 성능 후속 근거와 검증 정리`) 체크포인트 기준 완료 evidence 상태로 정리됐다: historical five lanes은 `open_confined` / `openat2`, `readdirplus` page/scan, matcher-heavy policy path, read/write follow-up beyond the completed fallback small-I/O baseline, mutation invalidation breadth-first다. `920c00e` 또는 dirty-worktree provenance를 가진 linked summary는 현재 baseline이 아니라 historical rejected/coverage context다. scope label 정의와 claim-grade gate는 [`benchmarks.md`](benchmarks.md)의 `Current next-priority candidate gates`가 source of truth이고, current evidence/gap snapshot은 [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md)에 정리한다. 이 run 이후 immediate next unfinished work는 `open_confined` / `openat2` frequency baseline completion이고, `lookup`/`getattr` same-path no-follow attr reuse는 현재 코드에 이미 들어간 dirty-worktree slice evidence로 별도 관리한다. 이후 later unfinished lanes은 `readdirplus` page/scan, matcher-heavy policy path, read/write follow-up beyond the completed fallback small-I/O baseline, mutation invalidation breadth-first다. 첫 pick이 여전히 `open_confined` / `openat2`인 이유는 checked-in fallback smoke가 attribution baseline일 뿐이고, rejected [`artifacts/open-confined-frequency/summary.md`](artifacts/open-confined-frequency/summary.md) bundle도 focused `metadata_opendir` improvement/counter reduction은 보여줬지만 claim-gate `open-confined-surface` rows를 통과하지 못해 kept speedup evidence가 아니기 때문이다. 이 row의 다음 instrumentation slice도 speedup claim이 아니라 `stat_child_no_follow` split attribution 보강으로 읽는다. 반대로 mutation invalidation lane은 breadth-first smoke/coverage 단계이며 fresh focused subtree baseline + `invalidations.scanned_entries` reduction이 먼저 필요하고, mounted post-`FORGET` observability는 별도 driver evidence가 있거나 limitation/rejected/coverage artifact로 명시돼야 한다.

| candidate | current scope | why this is still next | next gate |
| --- | --- | --- | --- |
| `open_confined` / `openat2` frequency | `smoke` | regenerated [`artifacts/current-open-confined-surface-smoke.md`](artifacts/current-open-confined-surface-smoke.md)와 [`artifacts/current-metadata-opendir-smoke.md`](artifacts/current-metadata-opendir-smoke.md)는 attribution coverage를 제공하고 `open_like.pre_open_guard.*` / `open_like.post_open_revalidation.*` surrounding-cost split까지 보여주며, rejected [`artifacts/open-confined-frequency/summary.md`](artifacts/open-confined-frequency/summary.md)는 focused fallback `metadata_opendir` improvement와 counter reduction이 가능함을 보여준다. Rejected [`artifacts/open-confined-fd-path/summary.md`](artifacts/open-confined-fd-path/summary.md)는 `/proc/self/fd/<fd>` builder allocation micro-candidate가 behavior-safe였어도 fallback `open-confined-surface`를 크게 regress시키고 fast tails/focused `metadata_opendir`도 gate를 놓쳐 reverted/not kept였음을 보여준다. 따라서 그 split과 rejected bundles 모두 `open-confined-surface` gate 실패를 뒤집는 speedup evidence는 아니며, 여전히 kept claim-grade speedup evidence는 없다. | fresh `open-confined-surface` pair on the candidate under test + explicit `--workload metadata_opendir` attribution/non-regression row when the question is opendir-specific |
| `readdirplus` page/scan | `slice` | [`artifacts/readdirplus-deferred-attrs/summary.md`](artifacts/readdirplus-deferred-attrs/summary.md)는 attr-work reduction만 입증하고 20k focused row는 약간 regress한다. [`artifacts/readdirplus-20k-policy-ablation/summary.md`](artifacts/readdirplus-20k-policy-ablation/summary.md)는 current **dirty worktree**에서 5k/20k와 fallback/fast를 갈라 본 coverage ablation이고, provenance에는 regenerated `current-open-confined-surface-smoke*` / `current-metadata-opendir-smoke*`, `scripts/bench-screenfs.py`, `src/fs.rs`, `src/fs/guards.rs`, `src/fs/perf.rs`, `src/fs/tests/perf.rs` 같은 open-like attribution/source changes가 포함된다. 따라서 isolated no-code-change control은 아니지만, focused 20k cost가 여전히 `readdirplus_directory_scan` 지배적이며 policy-sensitive임을 보여준다. 반면 [`artifacts/readdirplus-page-scan-path-join/summary.md`](artifacts/readdirplus-page-scan-path-join/summary.md)는 implementation change까지 섞인 rejected experiment다. [`artifacts/readdirplus-bounded-candidate-heap/summary.md`](artifacts/readdirplus-bounded-candidate-heap/summary.md)도 rejected page/scan evidence다: bounded candidate set은 20k p50만 개선했지만 p95/p99와 paired `directory-surface`/matcher32 non-regression gate를 놓쳐 reverted/not kept였다. page/scan retuning은 아직 미해결이다. | claim-grade `directory-surface` pair + focused large-directory `readdirplus_basic` row |
...
`open_confined/openat2`는 confinement correctness 핵심이라 대체하지 않는다. 대신 호출 빈도와 latency를 측정한다. The harness now includes `metadata_opendir` and `--workload-set open-confined-surface` so opendir/open-confined overhead can be separated from directory scan work. Regenerated [`artifacts/current-open-confined-surface-smoke.md`](artifacts/current-open-confined-surface-smoke.md) records mixed `metadata_open` + `metadata_opendir` perf-counter coverage with `--iterations 3 --warmups 1 --metadata-ops 200`, and regenerated [`artifacts/current-metadata-opendir-smoke.md`](artifacts/current-metadata-opendir-smoke.md) records a focused `--workload metadata_opendir` smoke. Both are measurement coverage, not speedup evidence, and both checked-in smokes are still fallback-only (`policy_preset=fallback-unsafe-policy`, `fast_path_cache_eligible=False`). Together they now show non-zero `open_like.pre_open_guard.*` / `open_like.post_open_revalidation.*` surrounding-cost splits around `open_confined_openat2`; read those counters as coverage/attribution for pre-open guard and post-open revalidation work, not as speedup evidence by themselves. The focused opendir smoke reports non-zero `fuse_op.opendir`, `open_confined_openat2`, `open_like.pre_open_guard.opendir`, `open_like.post_open_revalidation.opendir`, `source_root_path`, and `resolved_virtual_path_from_open_fd` counters without `readdir*` scan counters, but it does **not** prove `openat2` itself dominates; use [`artifacts/current-metadata-opendir-smoke.md`](artifacts/current-metadata-opendir-smoke.md) as the source of truth for the current relative totals, where `stat_child_no_follow` and `fuse_op.opendir` still exceed `open_confined_openat2` in that run.

현재 instrumentation split 계약:

- aggregate `stat_child_no_follow`는 계속 유지한다.
- 현재 `open_confined` measure-first lane의 attribution surface는 root-path branch용 `stat_child_no_follow.host_fstat`, non-root branch용 `stat_child_no_follow.parent_open` / `stat_child_no_follow.directory_revalidation` / `stat_child_no_follow.host_fstatat`, 공통 `stat_child_no_follow.attr_conversion`, 그리고 `stat_child_no_follow_context.*` context label(예: `path_guard_or_metadata`)을 포함한다. focused readlink perf coverage와 summary assertion은 `stat_child_no_follow_context.readlink_pre_open`도 별도로 확인한다.
- current fallback smokes 기준 다음 investigation/design target은 `stat_child_no_follow.directory_revalidation`이다. [`artifacts/current-metadata-opendir-smoke.md`](artifacts/current-metadata-opendir-smoke.md)에서는 `stat_child_no_follow.directory_revalidation.total_ns=15618029`가 같은 run의 `open_confined_openat2.total_ns=1804429`보다 크고 `stat_child_no_follow.total_ns=19736380` 대부분을 차지하며, [`artifacts/current-open-confined-surface-smoke.md`](artifacts/current-open-confined-surface-smoke.md)에서도 `40361300/71934271ns`로 같은 shape가 반복된다. 이 cost는 opened dirfd current-path validation이 `resolved_virtual_path_for_open_file(file)`와 `resolved_virtual_path(path, true)`를 둘 다 풀어 expected path와 fd path를 맞춰 보는 데서 나온다.
...
- 어떤 시도도 opened-dirfd current-path validation semantics를 유지하거나 같은 request 안에서의 equivalent proof를 제시해야 한다. cross-request cache는 금지하고, hidden `ENOENT`, symlink target visibility, `openat2` confinement도 그대로 유지한다.
...
Until a kept before/after pair clears that matrix, treat this row as the current next measure-first candidate rather than an implementation candidate.
...
- aggregate `stat_child_no_follow` count/latency (현재 `open_confined` measure-first surface는 aggregate를 유지하면서 root-path `host_fstat`, non-root `parent_open` / `directory_revalidation` / `host_fstatat`, 공통 `attr_conversion`, `stat_child_no_follow_context.*` 예: `path_guard_or_metadata`를 함께 내보낸다. focused readlink perf coverage는 `stat_child_no_follow_context.readlink_pre_open`도 확인한다)
...
3. 다음 성능 작업은 measure-first backlog에서 고르되, 현재 evidence 기준 첫 pick은 `open_confined/openat2` frequency baseline completion이다
...
5. 이후 later lanes는 `readdirplus` attr/page, matcher-heavy policy path, read/write follow-up, mutation invalidation breadth-first 순서로 유지한다
...
10. `open_confined/openat2`는 구현 전에 `--policy-preset fallback-unsafe-policy --workload-set open-confined-surface`, `--policy-preset fast-path-cache-eligible --workload-set open-confined-surface`, 필요 시 explicit `--workload metadata_opendir` baseline을 먼저 채운다
