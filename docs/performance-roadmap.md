...
이 section은 metadata/open-path 관련 work의 umbrella contract와 현재 slice 상태를 함께 정리한다. immediate next-priority ordering은 아래 `Current next-priority shortlist`와 [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md)를 따른다. 즉, `lookup`/`getattr` same-path no-follow attr reuse는 이미 현재 코드에 들어간 implemented slice로 별도 dirty-worktree slice evidence와 함께 읽고, immediate next unfinished work는 이제 matcher-heavy policy path다. Fast-policy/cache-eligible `readdirplus` page/scan은 claim-grade evidence를 확보했고, read-write/mutation lane을 후순위 measure-first row로 유지한다.
