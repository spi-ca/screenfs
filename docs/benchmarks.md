...
The evidence snapshot and scope labels for these five rows live in [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md). The `70a3105` all-five retry is now a completed historical checkpoint with rejected implementations documented under dedicated artifacts; these gates remain the source of truth for future refreshed attempts on the same five rows. The direct opened-object `stat_child_no_follow()` slice under `open_confined` is now covered by kept claim-grade evidence at [`artifacts/open-confined-stat-child-open-path/summary.md`](artifacts/open-confined-stat-child-open-path/summary.md), and the fast-policy/cache-eligible `readdirplus` page-scan fast path is now covered by kept claim-grade evidence at [`artifacts/readdirplus-page-scan-claim/summary.md`](artifacts/readdirplus-page-scan-claim/summary.md), so refreshed unfinished-work ordering now starts with matcher-heavy policy path. The already-implemented `lookup`/`getattr` same-path no-follow attr-reuse slice is tracked separately above as slice-scoped dirty-worktree evidence rather than as a future row in these five gates. The five historical rows below remain the source-of-truth gates for future refreshed attempts: the now-claimed fast-policy `readdirplus` and `open_confined` rows, plus unfinished matcher/read-write/mutation lanes. Unless a row says otherwise, the minimum claim-grade floor is a same-machine before/after pair with matching cache assumptions and at least:
