...
- Claim gate:
  - if a change expands `DirectoryChildVisibilityBatch` beyond the current `readdirplus` plus non-per-entry `readdir` all-visible/default-hidden-no-hidden-no-internal-hidden visible-subtree-anchor parent-local shapes, or changes rule-sensitive `readdir_scan.scan_visibility` / `readdirplus_scan.scan_visibility`, also satisfy the matcher-heavy evidence gate: repeated/interleaved same-shape series via `scripts/bench-screenfs-series.py`, a `matcher-descendant-directory` before/after row when descendant/bridge-visible behavior can change, and separate `directory-surface` / `directory-symlink-surface` rows when relevant
