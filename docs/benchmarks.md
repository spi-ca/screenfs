...
| `matcher_readonly_access_wok` | optional rule-rich readonly `W_OK` access probe enabled by `--matcher-extra-rules`; targets synthetic `matcher-heavy/visible-*/readonly-*.txt` files to exercise mutability matcher attribution without writes; measure-only, not a write throughput claim |
...
- perf-counter smoke/helper attribution from [`src/fs/tests/perf.rs`](../src/fs/tests/perf.rs) / `src/fs/tests/perf/**` (`perf_counters_split_resolved_virtual_path_sources` for the split counters, plus `perf_counters_record_data_path_splits_on_success` for size-bucket coverage)
...
With the current checked-in per-open-cache claim plus the kept direct opened-object `stat_child_no_follow()` speedup at [`artifacts/open-confined-stat-child-open-path/summary.md`](artifacts/open-confined-stat-child-open-path/summary.md), metadata/open-path fixed overhead remains one lane in the completed all-five-lane post-metadata follow-up checkpoint and in any future refreshed attempt, not the sole follow-up implementation target. The historical five-lane model captured under [`Current next-priority candidate gates`](#current-next-priority-candidate-gates) and [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md) is `open_confined` / `openat2`, `readdirplus` page/scan, matcher-heavy policy path, read/write follow-up beyond the completed fallback small-I/O baseline, and mutation invalidation breadth-first. Read the new open-confined artifact as a workload-scoped `open-confined-surface` claim for the direct opened-object metadata path, not as a blanket metadata/open-path win. For refreshed unfinished-work ordering, the already-implemented `lookup`/`getattr` same-path no-follow attr-reuse slice remains current dirty-worktree slice evidence under this umbrella, matcher-heavy no-writable mutability remains rejected evidence, and the active directory sequence is conservative `DirectoryChildVisibilityBatch` / `readdir*_scan.scan_visibility` expansion under strict policy-shape guardrails, then lazy child-path/allocation reduction for `readdir*_scan.name_child_path_materialization`, then very narrow `readdir*_candidate_selection` / `readdir*_page_commit` re-experiments, then a small cached-config skip-bit state-machine specialization slice. Keep bridge-visible carve-outs, direct-child glob matches, internal-hidden cases, and symlink target visibility fallback-safe throughout that sequence; later scoped lanes include `open_confined` / `metadata_opendir`, broader state-machine specialization beyond that cached-config slice, read/write, matcher trie/state invalidation, and mutation as measure-first work. This section defines the umbrella contract for the metadata/open-path row: fixed-overhead reduction on metadata/open-path operations (`lookup`, `getattr`, `open`, `readlink`, `access`) with `statfs` kept as a non-regression guardrail. Treat the current `small_stat_open_read` comparable workload as exploratory only for this surface: it is not split enough to support claim-grade attribution or acceptance by itself.
...
Directory claims must report `readdir_basic` and `readdirplus_basic` p50/p95/p99 together with `readdir_directory_scan`, `readdir_scan.*` sub-buckets, `readdir_attr_generation_scan`, `readdir_candidate_selection`, `readdir_page_commit`, `readdirplus_directory_scan`, `readdirplus_scan.*` sub-buckets, `readdirplus_attr_generation_scan`, `readdirplus_candidate_selection`, and `readdirplus_page_commit`. If symlink visibility logic changed, the regular-file directory fixture is insufficient; add `--workload-set directory-symlink-surface` or equivalent evidence that exercises `readdir_symlink_visibility` and `readdirplus_symlink_visibility`.
...
The evidence snapshot and scope labels for these five rows live in [`artifacts/current-next-performance-candidates.md`](artifacts/current-next-performance-candidates.md). The `70a3105` all-five retry is now a completed historical checkpoint with rejected implementations documented under dedicated artifacts; these gates remain the source of truth for future refreshed attempts on the same five rows. The direct opened-object `stat_child_no_follow()` slice under `open_confined` is covered by kept claim-grade evidence at [`artifacts/open-confined-stat-child-open-path/summary.md`](artifacts/open-confined-stat-child-open-path/summary.md), the fast-policy/cache-eligible `readdirplus` page-scan fast path is covered by kept claim-grade evidence at [`artifacts/readdirplus-page-scan-claim/summary.md`](artifacts/readdirplus-page-scan-claim/summary.md), while matcher-heavy no-writable mutability is rejected evidence at [`artifacts/matcher-no-writable-mutability-fast-path/summary.md`](artifacts/matcher-no-writable-mutability-fast-path/summary.md). Refreshed unfinished-work ordering now starts with conservative `DirectoryChildVisibilityBatch` / `readdir*_scan.scan_visibility` expansion under strict policy-shape guardrails, then lazy child-path/allocation reduction for `readdir*_scan.name_child_path_materialization`, then very narrow `readdir*_candidate_selection` / `readdir*_page_commit` re-experiments, then a small cached-config skip-bit state-machine specialization slice. Keep bridge-visible carve-outs, direct-child glob matches, internal-hidden cases, and symlink target visibility fallback-safe while expanding those batch shapes. Later scoped lanes include fallback `metadata_opendir` guard/stat attribution under `open_confined`, broader state-machine specialization beyond that cached-config slice, read/write follow-up, matcher trie/state invalidation ideas, and mutation invalidation breadth-first as later measure-first lanes. An explicit attribution-only slice may still reopen `readdirplus_basic` to inspect residual `returned_*`, but treat that as secondary smoke/attribution evidence rather than the active implementation order unless a later before/after implementation pair clears the `readdirplus` gate below. The already-implemented `lookup`/`getattr` same-path no-follow attr-reuse slice is tracked separately above as slice-scoped dirty-worktree evidence rather than as a future row in these five gates. The five historical rows below remain the source-of-truth gates for future refreshed attempts, but their subsection order is historical coverage order, not execution priority; use the refreshed unfinished-work ordering in this paragraph for the active pick sequence. Unless a row says otherwise, the minimum claim-grade floor is a same-machine before/after pair with matching cache assumptions and at least:
...
- Current label: `claim`
- Current evidence: [`artifacts/readdirplus-page-scan-claim/summary.md`](artifacts/readdirplus-page-scan-claim/summary.md) is the kept fast-policy/cache-eligible `readdirplus` page-scan claim for the conservative directory child visibility batch fast path. Fast `directory-surface` `readdirplus_basic` passes at p50/p95/p99 `0.844x`/`0.844x`/`0.845x`, the paired `readdir_basic` row improves at `0.122x`/`0.211x`/`0.263x`, and focused fast 20k `readdirplus_basic` passes at `0.587x`/`0.634x`/`0.631x`. The matcher32 row is context only for this fast-policy claim and its rerun showed no persistent rule-sensitive regression. Treat [`artifacts/readdirplus-page-scan-path-join/summary.md`](artifacts/readdirplus-page-scan-path-join/summary.md) as rejected page/scan retuning only: some focused 20k counters/tails improved, but 5k `readdir_basic` regressed badly (`1.557941x` p50), 20k `readdirplus_basic` p50 still regressed (`1.036241x`), and the implementation was reverted. Also treat [`artifacts/readdirplus-bounded-candidate-heap/summary.md`](artifacts/readdirplus-bounded-candidate-heap/summary.md) as rejected bounded candidate-selection evidence: 20k `readdirplus_basic` p50 improved (`0.882012x`) but p95/p99 missed (`0.998063x`/`1.002933x`), fast `directory-surface` `readdirplus_basic` regressed (`1.062710x`), matcher32 `readdir_basic` regressed (`1.480425x`), and the code was reverted/not kept. Also treat [`artifacts/readdirplus-returned-policy-recheck/summary.md`](artifacts/readdirplus-returned-policy-recheck/summary.md) as rejected returned-entry policy-recheck evidence: the experiment was behavior-safe in focused review, but fast `directory-surface` regressed on paired `readdir_basic` and missed the `readdirplus_basic` gate, matcher32 `readdirplus_basic` p50 regressed, focused 20k `readdirplus_basic` p50 regressed, and the Rust change was reverted/not kept.
- Required before/after rows:
  ...
  - store claim-grade artifacts separately from smoke/proof bundles, for example under `docs/artifacts/readdirplus-page-scan-claim/` with stems such as `before-<rev>-<policy-label>-directory-surface.*`, `after-<rev>-<policy-label>-directory-surface.*`, `before-<rev>-<policy-label>-readdirplus-basic-20k.*`, and matching `env-before|after-*` files when separate environment captures are produced
- Required counters:
  - `fuse_op.readdirplus`, `readdirplus_directory_scan`, `readdirplus_scan.*`, `readdirplus_attr_generation_scan`, `readdirplus_attr_generation_entries`, `readdirplus_candidate_selection`, `readdirplus_page_commit`
  - paired `readdir` non-regression rows must also report `fuse_op.readdir`, `readdir_directory_scan`, `readdir_scan.*`, `readdir_attr_generation_scan`, `readdir_candidate_selection`, `readdir_page_commit`
- Claim gate:
  - if a change expands `DirectoryChildVisibilityBatch` beyond the current `readdirplus` plus non-per-entry `readdir` all-visible/default-visible-subtree-hidden/no-internal-hidden parent-local/default-hidden-no-hidden-no-internal-hidden visible-subtree-anchor parent-local shapes, or changes rule-sensitive `readdir_scan.scan_visibility` / `readdirplus_scan.scan_visibility`, keep bridge-visible carve-outs, direct-child glob matches, internal-hidden cases, and symlink target visibility on the fallback-safe path, and also satisfy the matcher-heavy evidence gate: repeated/interleaved same-shape series via `scripts/bench-screenfs-series.py`, a `matcher-descendant-directory` before/after row when descendant/bridge-visible behavior can change, and separate `directory-surface` / `directory-symlink-surface` rows when relevant
  - for refreshed residual-directory work, prioritize conservative batch-classification changes first, then `readdir*_scan.name_child_path_materialization`; revisit `readdir*_candidate_selection` / `readdir*_page_commit` only under a narrow before/after matrix, and treat residual `returned_*` counters as secondary attribution unless that matrix exists
...
- Pre-implementation writeback/large-I/O note: the current repo keeps `write_back == false` by default and exposes `--experimental-writeback-cache` only as opt-in smoke/benchmark surface; it still has no repo-local `init()` override and no checked-in `FuseNotifier`/kernel cache invalidation path. Therefore any attempt that touches `writeback-cache`, widened host-open mode, or negotiated `max_write`/`max_readahead` must explicitly declare which pre-implementation track from [`performance-roadmap.md`](performance-roadmap.md) it exercises (`O_WRONLY` semantics, handle-mode separation, kernel cache invalidation scope, payload tuning, `fractal-fuse` `max_pages` caveat, sync gate) in [`artifacts/writeback-large-io-preflight/summary.md`](artifacts/writeback-large-io-preflight/summary.md), and include companion `--workload-set sync-surface` plus targeted `read-write-surface`/`per-open-cache-minimum` context rows before it can be read beyond planning/smoke. Runs must use an explicit `--policy-label` or artifact label containing `writeback-exp` or `large-io-exp` so opt-in mount experiments do not mix with baseline policy-preset rows, and must record observed/effective FUSE INIT payload values (`max_write`, `max_readahead`, `max_pages`, page-size assumption) when the experiment depends on negotiated payload size. Until repo-local capture tooling exists, collect these values from the startup log emitted by pinned `fractal-fuse` during `FUSE_INIT` when available, otherwise record dependency-source values from `fractal-fuse-0.4.0/src/session.rs` / `src/types.rs` plus local page size (`getconf PAGESIZE`) and mark the row as planning/smoke rather than claim-grade.
...
- Current label: `claim`
- Current caveat: the checked-in [`artifacts/open-confined-stat-child-open-path/summary.md`](artifacts/open-confined-stat-child-open-path/summary.md) bundle is a kept claim-grade `open-confined-surface` speedup for the direct opened-object `stat_child_no_follow()` path, not a blanket metadata/open-path win and not proof that `openat2` itself dominates the remaining cost. It passes both targeted rows on both policy presets: fallback `metadata_open` `0.559x`/`0.532x`/`0.516x`, fallback `metadata_opendir` `0.597x`/`0.567x`/`0.553x`, fast `metadata_open` `0.505x`/`0.596x`/`0.598x`, and fast `metadata_opendir` `0.646x`/`0.558x`/`0.562x` at p50/p95/p99. The regenerated [`artifacts/current-open-confined-surface-smoke.md`](artifacts/current-open-confined-surface-smoke.md) and [`artifacts/current-metadata-opendir-smoke.md`](artifacts/current-metadata-opendir-smoke.md) remain attribution-only context, and the rejected [`artifacts/open-confined-frequency/summary.md`](artifacts/open-confined-frequency/summary.md) / [`artifacts/open-confined-fd-path/summary.md`](artifacts/open-confined-fd-path/summary.md) bundles remain rejected alternatives rather than kept wins.
- Required before/after rows:
...
- Required counters:
  ...
  - the current split counter surface also includes direct opened-object metadata `stat_child_no_follow.host_fstat`, shared `stat_child_no_follow.attr_conversion`, and `stat_child_no_follow_context.*` labels such as `path_guard_or_metadata`; readlink preparation still checks `stat_child_no_follow.parent_open` / `stat_child_no_follow.directory_revalidation` / `stat_child_no_follow.host_fstatat` with `stat_child_no_follow_context.readlink_pre_open`
...
- Claim gate:
  - the checked-in direct opened-object `stat_child_no_follow()` change above clears this row for the current kept scope; future code changes on this lane must keep focused perf/correctness coverage for opened-dirfd revalidation plus the current direct-path/readlink split counters, then keep a dedicated before/after `open-confined-surface` pair and add explicit `metadata_opendir` when the question is opendir-specific
...
- Required counters:
  - `matcher_candidates`, `matcher_candidates_by_source.*`, `matcher_family_candidates.*`, `matcher_candidate_order.path`, `matcher_candidate_order.descendant`, `matcher_candidate_order_by_source.*`, `matcher_candidate_order_duplicates`, `matcher_candidate_order_seen_slots`, `matcher_candidate_order_ancestor_steps`
  - any targeted `fuse_op.*` rows for the claimed workload family
- Repeated/interleaved measurement helper:
  - use `scripts/bench-screenfs-series.py` as a thin wrapper around `scripts/bench-screenfs.py` when collecting matcher-heavy repeated evidence. It accepts `--before-bin`, `--after-bin`, `--pairs`, `--order before-after|after-before|alternate`, `--output-dir`, `--stem`, optional summarized `--workload` names, and passes the remaining arguments after `--` to the underlying harness.
  - for same-binary controls, pass the same binary to `--before-bin` and `--after-bin`; for candidate evidence, use explicitly built before/after binaries and labels. The wrapper writes pair-level child JSON/Markdown/SVG artifacts plus a manifest JSON and summary Markdown with per-pair after/before p50/p95/p99 ratios. [`artifacts/matcher-heavy-series-wrapper-smoke/summary.md`](artifacts/matcher-heavy-series-wrapper-smoke/summary.md) records a minimal wrapper smoke; it is tooling evidence only, not performance evidence. [`artifacts/matcher-heavy-series-control/worktree-8720eb8/summary.md`](artifacts/matcher-heavy-series-control/worktree-8720eb8/summary.md) records a 4-pair alternate same-binary control using the wrapper and is also control/no-claim evidence.
- Claim gate:
  - because [`artifacts/matcher-heavy-no-code-control/summary.md`](artifacts/matcher-heavy-no-code-control/summary.md) and [`artifacts/matcher-heavy-descendant-no-code-control/summary.md`](artifacts/matcher-heavy-descendant-no-code-control/summary.md) show large same-binary within-pair movement, matcher-heavy claims must use repeated or interleaved same-shape before/after pairs rather than a single pair
  - the matcher-heavy row must show after/before `<= 0.90x` median and `<= 0.95x` at p95/p99 for `matcher_hidden_stat_miss` plus at least one claimed metadata or directory workload in the same policy row, with stable direction across the repeated/interleaved pairs and outside the same-binary control envelope
